...
CADAGUC_time::CADAGUC_time(const char * DateSince){
 ...
 // strncpy(stpszDateSince,DateSince,MAX_STR_LEN);stpszDateSince[MAX_STR_LEN]='\0';
  if (utInit("") != 0) {
...
int CADAGUC_time::HDF4DateTimeToADAGUC(stADAGUC_time  &ADtime, int date_in,int time_in){
  snprintf(szTime,MAX_STR_LEN,"%08dT%06d",date_in,time_in);
...
int CADAGUC_time::ISOTimeToADAGUC(stADAGUC_time  &ADtime,const char *szTime){
  ...
  snprintf(szTime_test_2,MAX_STR_LEN,"%04d-%02d-%02dT%02d:%02d:%02d",check.year,check.month,check.day,check.hour,check.minute,(int)check.second);
  if(strncmp(szTime,szTime_test_2,15)!=0){
    snprintf(szTemp,MAX_STR_LEN,"Time Error in ISOTimeToADAGUC: inTime != testTime: \"%s\" != \"%s\"",szTime,szTime_test_2);
    CDBError(szTemp);
    return 1; 
  }
  return 0;
...
//  double offset;
  if(utInvCalendar(ADtime.year,ADtime.month,
     ADtime.day,ADtime.hour,ADtime.minute,
     (int)ADtime.second,&dataunits,&ADtime.offset) != 0){
       snprintf(szTemp,MAX_STR_LEN,"YYYYMMDDTHHMMSSToADAGUC: Internal error: utInvCalendar : [%s]",szTime);
     ...
     snprintf(szTime_test_2,MAX_STR_LEN,"%04d%02d%02dT%02d%02d%02d",check.year,check.month,check.day,check.hour,check.minute,(int)check.second);
     if(strncmp(szTime,szTime_test_2,15)!=0){
       snprintf(szTemp,MAX_STR_LEN,"Time Error in YYYYMMDDTHHMMSSToADAGUC: inTime != testTime: \"%s\" != \"%s\"",szTime,szTime_test_2);
       CDBError(szTemp);
       return 1; 
     }
     return 0;
