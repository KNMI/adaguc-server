...
## ADAGUC_AUTOREMOVENONMATCHINGFILES

ADAGUC_AUTOREMOVENONMATCHINGFILES only comes into action when scanning a file with the scan script using `-f` option. If the file does not belong to any dataset, and ADAGUC_AUTOREMOVENONMATCHINGFILES=TRUE, the file will be removed to avoid cluttering the filesystem. 
