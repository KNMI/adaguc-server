...
```
docker exec -i -t my-adaguc-server scan.sh -d mymonthlyconfig
```
...
```
docker exec -i -t my-adaguc-server scan.sh -d mymonthlyconfig apr.nc
```

If you would like to scan specific files in a certain subdirectory of
the configured data directory, you can do:
```
docker exec -i -t my-adaguc-server scan.sh -d mymonthlyconfig 2013
```
