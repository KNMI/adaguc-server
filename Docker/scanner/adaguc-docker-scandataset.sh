...
# 2020-04-17
#
# This script is for scanning datasets (XML configs) with adaguc containers.
# This script calls the adaguc-server container to scan the specified dataset. 
# This script uses the settings as provided in the Docker/.env file.
#
# Usage: bash ./Docker/scanner/adaguc-docker-scandataset.sh <datasetname>
# Example: bash ./Docker/scanner/adaguc-docker-scandataset.sh eobs
#
...
docker run -it --rm --network docker_adaguc-network \
    ...
    --entrypoint '/adaguc/adaguc-server-updatedatasets.sh' \
