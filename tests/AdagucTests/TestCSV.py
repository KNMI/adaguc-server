...
class TestCSV(unittest.TestCase):
    ...
    env = {'ADAGUC_CONFIG': ADAGUC_PATH + "/data/config/adaguc.tests.dataset.xml," +
           ADAGUC_PATH + "/data/config/datasets/adaguc.testCSVReader.xml"}
    ...
    def test_CSV_12timesteps(self):
        ...
        status, data, headers = AdagucTestTools().runADAGUCServer(
            args=['--updatedb', '--config', config], env=self.env, isCGI=False, showLog=False)
        self.assertEqual(status, 0)
        ...
        for date in dates:
            ...
            status, data, headers = AdagucTestTools().runADAGUCServer(
                "&SERVICE=WMS&&SERVICE=WMS&VERSION=1.3.0&REQUEST=GetMap&LAYERS=wind&WIDTH=256&HEIGHT=256&CRS=EPSG%3A4326&BBOX=-180,-90,180,90&STYLES=windbarb&FORMAT=image/png&TRANSPARENT=TRUE&TIME=" + date, env=self.env)
            AdagucTestTools().writetofile(self.testresultspath + filename, data.getvalue())
            self.assertEqual(status, 0)
    ...
    def test_CSV_reference_time(self):
        AdagucTestTools().cleanTempDir()
        env = {'ADAGUC_CONFIG': ADAGUC_PATH + "/data/config/adaguc.tests.dataset.xml," +
               ADAGUC_PATH + "/data/config/datasets/adaguc.testCSVReader_reference_time.xml"}
        config = ADAGUC_PATH + '/data/config/adaguc.tests.dataset.xml,' + \
            ADAGUC_PATH + '/data/config/datasets/adaguc.testCSVReader_reference_time.xml'
        status, data, headers = AdagucTestTools().runADAGUCServer(
    ...
    def test_CSV_reference_time_GetMap(self):
        ...
        for testcase in testcases:
            ...
            status, data, headers = AdagucTestTools().runADAGUCServer("&SERVICE=WMS&&SERVICE=WMS&VERSION=1.3.0&REQUEST=GetMap&LAYERS=wind&WIDTH=256&HEIGHT=256&CRS=EPSG%3A4326&BBOX=-180,-90,180,90&STYLES=windbarb&FORMAT=image/png&TRANSPARENT=TRUE&TIME=" +
                                                                      date + "&DIM_reference_time=" + DIM_reference_time, env=env)
            AdagucTestTools().writetofile(self.testresultspath + filename, data.getvalue())
            self.assertEqual(status, 0)
            self.assertEqual(data.getvalue(), AdagucTestTools(
            ).readfromfile(self.expectedoutputsspath + filename))
    ...
    def test_CSV_negative_values(self):
        ...
        status, data, headers = AdagucTestTools().runADAGUCServer(
            "&SERVICE=WMS&&SERVICE=WMS&VERSION=1.3.0&REQUEST=GetMap&LAYERS=tn&WIDTH=256&HEIGHT=256&CRS=EPSG%3A3857&BBOX=4000904.446200706,-1231688.3419664246,4468921.645102525,-685668.2765809689&STYLES=name/point&FORMAT=image/png&TRANSPARENT=TRUE&", env=self.env)
        AdagucTestTools().writetofile(self.testresultspath + filename, data.getvalue())
        self.assertEqual(status, 0)
        self.assertEqual(data.getvalue(), AdagucTestTools(
        ).readfromfile(self.expectedoutputsspath + filename))
    ...
    def test_CSV_radiusandvalue(self):
        ...
        status, data, headers = AdagucTestTools().runADAGUCServer(
            "&SERVICE=WMS&&SERVICE=WMS&VERSION=1.3.0&REQUEST=GetMap&LAYERS=radiusandvalue&WIDTH=256&HEIGHT=512&CRS=EPSG%3A3857&BBOX=-8003558.6330057755,1638420.481402514,-7346556.700484946,2491778.5155690867&STYLES=magnitude&FORMAT=image/png&TRANSPARENT=TRUE&showlegend=true", env=env)
        AdagucTestTools().writetofile(self.testresultspath + filename, data.getvalue())
        self.assertEqual(status, 0)
        self.assertEqual(data.getvalue(), AdagucTestTools(
        ).readfromfile(self.expectedoutputsspath + filename))
