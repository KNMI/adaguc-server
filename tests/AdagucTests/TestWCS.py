...
#                      b'II*\x00\x08\x00\x00\x00\x12\x00')
  ...
  def test_WCSGetCoverageAAIGRID_NATIVE_testdatanc(self):
    ...
    print("\n----")
    print(re.split(r'\s+', lines[0])[0])
    
    # ncols        29
    # nrows        31
    # xllcorner    -1500000.013392857043
    # yllcorner    -4206896.551190475933
    # cellsize     103448.276785714275
    # NODATA_value  -0.0099999997764825820923
    assert re.split(r'\s+', lines[0])[0] == 'ncols'
    assert re.split(r'\s+', lines[0])[1] == '29'
    
    assert re.split(r'\s+', lines[1])[0] == 'nrows'
    assert re.split(r'\s+', lines[1])[1] == '31'
    
    assert re.split(r'\s+', lines[2])[0] == 'xllcorner'
    self.assertEqual(round(float(re.split(r'\s+', lines[2])[1]),2), -1500000.01)
    
    assert re.split(r'\s+', lines[3])[0] == 'yllcorner'
    self.assertEqual(round(float(re.split(r'\s+', lines[3])[1]),2), -4206896.55)

    assert re.split(r'\s+', lines[4])[0] == 'cellsize'
    assert re.split(r'\s+', lines[4])[1], '103448.276785714275'
    self.assertEqual(round(float(re.split(r'\s+', lines[4])[1]),2), 103448.28)

    assert re.split(r'\s+', lines[5])[0] == 'NODATA_value'
    self.assertEqual(round(float(re.split(r'\s+', lines[5])[1]),3), -0.01)
