  ...
  """
    ...
    def test_WCSGetCapabilities_testdatanc(self):
        ...
        status, data, headers = AdagucTestTools().runADAGUCServer(
            "source=testdata.nc&SERVICE=WCS&request=getcapabilities",
            env=self.env)
    ...
    def test_WCSGetCoverageAAIGRID_testdatanc(self):
        ...
        expected_output = AdagucTestTools().readfromfile(
            self.expectedoutputsspath + filename)
    ...
    def test_WCSGetCoverageNetCDF4_testdatanc_adaguc_wcs_destgridspec_specifiedgridresxresy(
            self):
        """
    ...
    def test_WCSGetCoverageNetCDF4_testdatanc_adaguc_wcs_destgridspec_specifiedgridwidthheight(
            self):
        """
        Check if WCS GetCoverage for specified settings returns correct grid spec
        """
    ...
    def test_WCSGetCoverageNetCDF4_testdatanc_adaguc_wcs_destgridspec_native(
            self):
        """
        Check if WCS GetCoverage for specified settings returns correct grid spec
        """
        AdagucTestTools().cleanTempDir()
        status, data, headers = AdagucTestTools().runADAGUCServer(
            "source=testdata.nc&SERVICE=WCS&REQUEST=GetCoverage&COVERAGE=testdata&FORMAT=NetCDF4",
            env=self.env,
            args=["--report"])
        self.assertEqual(status, 0)
        self.assertEqual(data.getvalue()[0:6], b'\x89HDF\r\n')
    ...
    def test_WCSGetCoverageNetCDF4_testdatanc_adaguc_wcs_destgridspec_specifiedgrid_noresxnoresy_fullglobe(
            self):
        """
        Check if WCS GetCoverage for specified settings returns correct grid spec
        """
    ...
    def test_WCSGetCoverageNetCDF4_testdatanc_adaguc_wcs_destgridspec_specifiedgrid_noresxnoresy_nl(
            self):
        """
        Check if WCS GetCoverage for specified settings returns correct grid spec
        """
    ...
    def test_WCSGetCoverageNetCDF4_testdatanc_adaguc_wcs_destgridspec_specifiedgridwidthheight_responsecrs(
            self):
        """
        Check if WCS GetCoverage for specified settings returns correct grid spec
        """
    ...
    def test_WCS_WithCaching(self):
        ...
        status, data, headers = AdagucTestTools().runADAGUCServer(
            "SERVICE=WCS&request=DescribeCoverage&coverage=data",
        ...
        with tempfile.NamedTemporaryFile() as tmp:
            ...
            self.assertEqual(dataset.variables["member"][0],
                             "member6")  # Largest value (default)
        ...
        self.assertTrue("Content-Type:application/netcdf" in headers)
        self.assertTrue(
            "Cache-Control:max-age=7200" in
            headers)  # Fully specified with the fixed dimension and the query

        with tempfile.NamedTemporaryFile() as tmp:
            tmp.write(data.getvalue())
            dataset = Dataset(tmp.name, mode='r')
            self.assertEqual(dataset.variables["member"][0],
                             "member4")  # Fixed dimension
