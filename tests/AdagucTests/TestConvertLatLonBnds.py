...
class TestConvertLatLonBnds(unittest.TestCase):
    ...
    def test_ConvertLatLonBnds_getMap(self):
        ...
        status, data, headers = AdagucTestTools().runADAGUCServer(
            "source=example_file_latlonbnds.nc&SERVICE=WMS&SERVICE=WMS&VERSION=1.3.0&REQUEST=GetMap&LAYERS=probability&WIDTH=512&HEIGHT=512&CRS=EPSG%3A4326&BBOX=50,2,57,9&STYLES=auto%2Fnearest&FORMAT=image/png&TRANSPARENT=FALSE&showlegend=true&DIM_threshold=20&time=2024-06-01T00:00:00Z",
            env=self.env,
            args=["--report"],
        )
        AdagucTestTools().writetofile(self.testresultspath + filename, data.getvalue())
