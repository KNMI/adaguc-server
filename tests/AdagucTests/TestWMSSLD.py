...
class TestWMSSLD(unittest.TestCase):
    ...
    def test_WMSGetMap_testdatanc(self):
        ...
        status,data,headers = AdagucTestTools().runADAGUCServer("SERVICE=WMS&SERVICE=WMS&VERSION=1.3.0&REQUEST=GetMap&LAYERS=radar&WIDTH=256&HEIGHT=256&CRS=EPSG%3A4326&BBOX=30,-30,75,30&STYLES=testdata%2Fnearest&FORMAT=image/png&TRANSPARENT=FALSE&",
                                                                env = self.env)
    ...
    def test_WMSGetMapSLD_NOSLDURL_testdatanc(self):
        AdagucTestTools().cleanTempDir()
        filename="test_WMSGetMap_testdatanc_NOSLDURL.xml"
        status,data,headers = AdagucTestTools().runADAGUCServer("source=testdata.nc&SERVICE=WMS&SERVICE=WMS&VERSION=1.3.0&REQUEST=GetMap&LAYERS=radar&WIDTH=256&HEIGHT=256&CRS=EPSG%3A4326&BBOX=30,-30,75,30&STYLES=testdata%2Fnearest&FORMAT=image/png&TRANSPARENT=FALSESLD=",
