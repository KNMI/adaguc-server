        ...
        if (csld.parameterIsSld(values[0])) {
          ...
          if (csld.serverConfigCheck(srvParam->cfg)) {
            ...
            if (!values[1].empty()) {
              ...
              //Process SLD file
              status = csld.processSLD(values[1]);
              ...
              CDBError("SLD parameter needs to contain a file name");
            }
          }
        }
      }
  ...
  for (size_t j = 0; j < srvParam->cfg->Layer.size(); j++) {
    if (srvParam->cfg->Layer[j]->attr.type.equals("autoscan")) {
      ...
      try {
        ...
        try {
          fileList = CDBFileScanner::searchFileNames(baseDir, srvParam->cfg->Layer[j]->FilePath[0]->attr.filter.c_str(),
                                                     NULL);
        } catch (int linenr) {
        ...
        for (size_t j = 0; j < fileList.size(); j++) {
          try {
            ...
            CDFObject *cdfObject = CDFObjectStore::getCDFObjectStore()->getCDFObjectHeader(srvParam,
                                                                                           fileList[j].c_str());
            if (cdfObject == NULL) {
  ...
  if (hasReferenceTimeDimension) {
    ...
    try {
      tableName = CDBFactory::getDBAdapter(srvParam->cfg)->getTableNameForPathFilterAndDimension(
        dataSource->cfgLayer->FilePath[0]->value.c_str(), dataSource->cfgLayer->FilePath[0]->attr.filter.c_str(),
        dimName.c_str(), dataSource);
    } catch (int e) {
      CDBError("Unable to create tableName from '%s' '%s' '%s'", dataSource->cfgLayer->FilePath[0]->value.c_str(),
               dataSource->cfgLayer->FilePath[0]->attr.filter.c_str(), dimName.c_str());
      return 1;
    }

    CDBStore::Store *store = CDBFactory::getDBAdapter(srvParam->cfg)->getUniqueValuesOrderedByValue(dimName.c_str(), -1,
                                                                                                    false,
                                                                                                    tableName.c_str());
    if (store == NULL) {
    ...
    for (size_t j = 0; j < dataSource->srvParams->cfg->Layer.size(); j++) {
      ...
      if (groupName.testRegEx(
        "[[:digit:]][[:digit:]][[:digit:]][[:digit:]][[:digit:]][[:digit:]][[:digit:]][[:digit:]]_[[:digit:]][[:digit:]]")) {
        CT::string ymd = groupName.substring(0, 8);
        CT::string hh = groupName.substring(9, 11);
        ymd.concat(hh);
        ymd.concat("00");
        WMSGroups.insert(ymd.c_str());
      } else if (groupName.testRegEx(
  ...
  if (cacheNeedsRefresh == false) {
    ...
    if (simpleStore->getCTStringAttribute("configModificationDate", &oldConfigModificationDate) != 0) {
      ...
      if (oldConfigModificationDate.equals(&configModificationDate)) {
        ...
        CDBDebug("Cache needs update because %s!=%s", oldConfigModificationDate.c_str(),
                 configModificationDate.c_str());
        //simpleStore->setStringAttribute("configModificationDate",configModificationDate.c_str());
        cacheNeedsRefresh = true;
      }
    }
...
int CRequest::setDimValuesForDataSource(CDataSource *dataSource, CServerParams *srvParam) {
  ...
  if (dataSource->cfgLayer->Dimension.size() == 0) {
    ...
    std::vector <std::string> fileList;
    try {
      fileList = CDBFileScanner::searchFileNames(dataSource->cfgLayer->FilePath[0]->value.c_str(),
                                                 dataSource->cfgLayer->FilePath[0]->attr.filter, NULL);
    } catch (int linenr) {
          ...
          if (srvParam->requestDims[k]->name.equals(&dimName)) {
            ...
            if (ogcDim->name.equals("time") || ogcDim->name.equals("reference_time")) {
              ...
              if (dataSource->requiredDims[i]->value.charAt(10) == 'T') {
                ...
                if (dataSource->requiredDims[i]->value.indexOf("/") == -1 &&
                    dataSource->requiredDims[i]->value.indexOf(",") == -1) {
#ifdef CREQUEST_DEBUG
                  ...
                  try {
                    currentTimeAsEpoch = ctime.dateToOffset(
                      ctime.freeDateStringToDate(dataSource->requiredDims[i]->value.c_str()));
              ...
              if (!dataSource->cfgLayer->Dimension[i]->attr.quantizeperiod.empty()) {
                CDBDebug("For dataSource %s found quantizeperiod %s", dataSource->layerName.c_str(),
                         dataSource->cfgLayer->Dimension[i]->attr.quantizeperiod.c_str());
            ...
            if (ogcDim->value.equals("current")) {
              ...
              try {
                tableName = CDBFactory::getDBAdapter(srvParam->cfg)->getTableNameForPathFilterAndDimension(
                  ...
                  dataSource->cfgLayer->FilePath[0]->attr.filter.c_str(), ogcDim->netCDFDimName.c_str(), dataSource);
              } catch (int e) {
                CDBError("Unable to create tableName from '%s' '%s' '%s'",
                         dataSource->cfgLayer->FilePath[0]->value.c_str(),
                         dataSource->cfgLayer->FilePath[0]->attr.filter.c_str(), ogcDim->netCDFDimName.c_str());
                return 1;
              }
              ...
              if (hasReferenceTimeDimension == false) {
                ...
                CDBStore::Store *maxStore = CDBFactory::getDBAdapter(srvParam->cfg)->getMax(
                  ogcDim->netCDFDimName.c_str(), tableName.c_str());
                if (maxStore == NULL) {
                ...
                if (dataSource->cfgLayer->Dimension[i]->value.equals("time")) {
                  CDBStore::Store *maxStore = CDBFactory::getDBAdapter(srvParam->cfg)->getClosestDataTimeToSystemTime(
                    ogcDim->netCDFDimName.c_str(), tableName.c_str());
    ...
    for (size_t i = 0; i < dataSource->cfgLayer->Dimension.size(); i++) {
      ...
      if (alreadyAdded == false) {
        ...
        try {
          ...
          CDBError("Unable to create tableName from '%s' '%s' '%s'", dataSource->cfgLayer->FilePath[0]->value.c_str(),
                   dataSource->cfgLayer->FilePath[0]->attr.filter.c_str(), netCDFDimName.c_str());
          return 1;
        }
        ...
        if (!isReferenceTimeDimension) {
          ...
          maxStore = CDBFactory::getDBAdapter(srvParam->cfg)->getMax(
            dataSource->cfgLayer->Dimension[i]->attr.name.c_str(), tableName.c_str());
        } else {
          ...
          for (size_t j = 0; j < dataSource->requiredDims.size(); j++) {
            CDBDebug("DIMS: %d [%s] [%s]", j, dataSource->requiredDims[j]->name.c_str(),
                     dataSource->requiredDims[j]->value.c_str());
          ...
          if (timeValue.empty()) {
            ...
            try {
              timeTableName = CDBFactory::getDBAdapter(srvParam->cfg)->getTableNameForPathFilterAndDimension(
                dataSource->cfgLayer->FilePath[0]->value.c_str(),
                dataSource->cfgLayer->FilePath[0]->attr.filter.c_str(), netcdfTimeDimName.c_str(), dataSource);
            } catch (int e) {
              CDBError("Unable to create tableName from '%s' '%s' '%s'",
                       dataSource->cfgLayer->FilePath[0]->value.c_str(),
                       dataSource->cfgLayer->FilePath[0]->attr.filter.c_str(), netcdfTimeDimName.c_str());
              return 1;
            }

            maxStore = CDBFactory::getDBAdapter(srvParam->cfg)->getReferenceTime(ogcDim->netCDFDimName.c_str(),
...
//       }
      ...
      if (tileSettingsDebug) {
        ...
        srvParam->mapSubTitle.print("level %d, tiles %0d, tileCellSize %f, screenCellSize %f", dataSource->queryLevel,
                                    store->getSize(), tileCellSize, screenCellSize);
      }
    ...
    for (size_t j = 0; j < srvParam->WMSLayers->count; j++) {
      ...
      for (layerNo = 0; layerNo < srvParam->cfg->Layer.size(); layerNo++) {
        ...
        if (layerName.equals(srvParam->WMSLayers[j].c_str())) {
          ...
          if (dataSource->setCFGLayer(srvParam, srvParam->configObj->Configuration[0], srvParam->cfg->Layer[layerNo],
                                      layerName.c_str(), j) != 0) {
            return 1;
          }
          ...
          for (size_t additionalLayerNr = 0;
               additionalLayerNr < srvParam->cfg->Layer[layerNo]->AdditionalLayer.size(); additionalLayerNr++) {
            ...
            for (additionalLayerNo = 0; additionalLayerNo < srvParam->cfg->Layer.size(); additionalLayerNo++) {
              ...
              if (additionalLayerName.equals(additional)) {
                ...
                if (additionalDataSource->setCFGLayer(srvParam, srvParam->configObj->Configuration[0],
                                                      srvParam->cfg->Layer[additionalLayerNo],
                                                      additionalLayerName.c_str(), j) != 0) {
                ...
                checkForData->setCFGLayer(srvParam, srvParam->configObj->Configuration[0],
                                          srvParam->cfg->Layer[additionalLayerNo], additionalLayerName.c_str(), j);
                try {
                ...
                CDBDebug("add = %d replaceAllDataSource = %d replacePreviousDataSource = %d", add, replaceAllDataSource,
                         replacePreviousDataSource);
                if (add) {
      ...
      if (dataSources[j]->cfgLayer->Dimension.size() != 0) {
        try {
          if (setDimValuesForDataSource(dataSources[j], srvParam) != 0) {
            CDBError("Error in setDimValuesForDataSource: Unable to find data for layer %s",
                     dataSources[j]->layerName.c_str());
            return 1;
          }
        } catch (ServiceExceptionCode e) {
        ...
        std::vector <std::string> fileList;
        try {
          fileList = CDBFileScanner::searchFileNames(dataSources[j]->cfgLayer->FilePath[0]->value.c_str(),
                                                     dataSources[j]->cfgLayer->FilePath[0]->attr.filter, NULL);
        } catch (int linenr) {
          CDBError("Could not find any filename");
          return 1;
        }
    ...
    if (srvParam->dFound_BBOX == 0) {
      for (size_t d = 0; d < dataSources.size(); d++) {
        if (dataSources[d]->dLayerType != CConfigReaderLayerTypeCascaded &&
          ...
          warper.closereproj();
          CDBDebug("Found bbox %s %f %f %f %f", srvParam->Geo->CRS.c_str(), srvParam->Geo->dfBBOX[0],
                   srvParam->Geo->dfBBOX[1], srvParam->Geo->dfBBOX[2], srvParam->Geo->dfBBOX[3]);
      ...
      if (srvParam->requestType == REQUEST_WMS_GETMAP) {
        ...
        if (useThreading) {
          ...
          for (size_t k = 0; k < (size_t) dataSources[dataSourceToUse]->getNumTimeSteps(); k++) {
            ...
            if (dataSources[j]->dLayerType == CConfigReaderLayerTypeDataBase ||
              ...
              if (status != 0) {
                ...
                if (dataSources[dataSourceToUse]->getNumTimeSteps() == 1) {
                  ...
                  CDBError("Unable to load datasource %s at line %d",
                           dataSources[dataSourceToUse]->getDataObject(0)->variableName.c_str(), __LINE__);
                }
              }
            }
            ...
            if (dataSources[dataSourceToUse]->getNumTimeSteps() > 1 &&
                dataSources[dataSourceToUse]->queryBBOX == false) {
              //Print the animation data into the image
              char szTemp[1024];
              snprintf(szTemp, 1023, "%s UTC",
                       dataSources[dataSourceToUse]->getDimensionValueForNameAndStep("time", k).c_str());
        ...
        if (srvParam->showDimensionsInImage) {
          ...
          for (size_t d = 0; d < nDims; d++) {
            ...
            message.print("%s: %s", dataSource->requiredDims[d]->name.c_str(),
                          dataSource->requiredDims[d]->value.c_str());
            imageDataWriter.drawImage.drawText(6, textY, srvParam->cfg->WMS[0]->DimensionFont[0]->attr.location.c_str(),
                                               fontSize, 0, message.c_str(), CColor(0, 0, 0, 255),
  ...
  for (size_t j = 0; j < parameters->count; j++) {
    ...
    if (values->count >= 2) {
      ...
      if (foundDim != -1) {
        ...
        } else { CDBDebug("OGC Dim %s reused", ogcDimName); }
  ...
  if (dErrorOccured == 0 && srvParam->serviceType == SERVICE_WMS) {
    ...
    if (srvParam->requestType == REQUEST_WMS_GETMAP || srvParam->requestType == REQUEST_WMS_GETLEGENDGRAPHIC) {
      if (dFound_Format == 0) {
        ...
        if (srvParam->Format.indexOf("32") > 0) {
          srvParam->imageFormat = IMAGEFORMAT_IMAGEPNG32;
          srvParam->imageMode = SERVERIMAGEMODE_RGBA;
        } else if (srvParam->Format.indexOf("24") > 0) {
          srvParam->imageFormat = IMAGEFORMAT_IMAGEPNG24;
          srvParam->imageMode = SERVERIMAGEMODE_RGBA;
        } else if (srvParam->Format.indexOf("8bit_noalpha") > 0) {
          srvParam->imageFormat = IMAGEFORMAT_IMAGEPNG8_NOALPHA;
          srvParam->imageMode = SERVERIMAGEMODE_RGBA;
        } else if (srvParam->Format.indexOf("png8_noalpha") > 0) {
          srvParam->imageFormat = IMAGEFORMAT_IMAGEPNG8_NOALPHA;
          srvParam->imageMode = SERVERIMAGEMODE_RGBA;
        } else if (srvParam->Format.indexOf("8") > 0) {
          srvParam->imageFormat = IMAGEFORMAT_IMAGEPNG8;
          srvParam->imageMode = SERVERIMAGEMODE_RGBA;
        } else if (srvParam->Format.indexOf("webp") > 0) {
          srvParam->imageFormat = IMAGEFORMAT_IMAGEWEBP;
          srvParam->imageMode = SERVERIMAGEMODE_RGBA;
        } else if (srvParam->Format.indexOf("gif") > 0) {
...
//        dErrorOccured=1;
      ...
      if (dFound_Width == 0) {
        if (srvParam->Geo->dfBBOX[2] != srvParam->Geo->dfBBOX[0]) {
          float r = fabs(srvParam->Geo->dfBBOX[3] - srvParam->Geo->dfBBOX[1]) /
                    fabs(srvParam->Geo->dfBBOX[2] - srvParam->Geo->dfBBOX[0]);
  ...
  if (dErrorOccured == 0 && srvParam->serviceType == SERVICE_WCS) {
    ...
    if (dErrorOccured == 0 && srvParam->requestType == REQUEST_WCS_GETCOVERAGE) {
      CDBDebug("WCS");
      if (dFound_Width == 0 && dFound_Height == 0 && dFound_RESX == 0 && dFound_RESY == 0 &&
          srvParam->dFound_BBOX == 0 && dFound_CRS == 0)
  ...
  if (srvParam->serviceType == SERVICE_WCS) {
    CDBError(
      "ADAGUC Server: Invalid value for request. Supported requests are: getcapabilities, describecoverage and getcoverage");
  } else if (srvParam->serviceType == SERVICE_WMS) {
    CDBError(
      "ADAGUC Server: Invalid value for request. Supported requests are: getcapabilities, getmap, gethistogram, getfeatureinfo, getpointvalue, getmetadata, getReferencetimes, getstyles and getlegendgraphic");
  } else {
