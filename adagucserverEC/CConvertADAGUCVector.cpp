  ...
  if(mode == CNETCDFREADER_MODE_OPEN_ALL) {
    ...
    if(dataSource->srvParams->Geo->CRS.length() > 0) {
      ...
      if(cdfObject->getVariableNE("customgridprojection") == NULL) {
        ...
        imageWarper.decodeCRS(&dataSource->nativeProj4, &dataSource->nativeEPSG,
                              &dataSource->srvParams->cfg->Projection);
    ...
    if(projectionRequired) {
      int status = imageWarper.initreproj(dataSource, dataSource->srvParams->Geo,
                                          &dataSource->srvParams->cfg->Projection);
      if(status != 0) {
