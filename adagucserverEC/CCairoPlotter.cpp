...
void CCairoPlotter::drawStrokedText(int x, int y, double angle, const char *text, VectorStyle vectorStyle) {
  if (library == NULL) {
    int status = initializeFreeType();
    if (status != 0) {
      // TODO
  ...
  cairo_set_font_size(cr, vectorStyle.fontSize);
  ...
  if (vectorStyle.outlineWidth > 0) {
    CDBDebug("@@@: strokeWidth=%f", vectorStyle.textOutlineWidth);
    CColor bgcolor = vectorStyle.textOutlineColor;

    vectorStyle.outlineWidth /= 2;
    // strokeWidth = 2;
    // CColor myColor(0, 255, 0, 255);
    cairo_set_antialias(cr, CAIRO_ANTIALIAS_DEFAULT);
    ...
    cairo_set_line_width(cr, 2.5 + vectorStyle.textOutlineWidth);
    ...
    cairo_set_line_width(cr, 1.5 + vectorStyle.textOutlineWidth);
    cairo_stroke_preserve(cr);
  }

  CColor fgcolor = vectorStyle.textColor;
...
void CCairoPlotter::drawBarb(int x, int y, double uncorrectedDirection, double viewDirCorrection, double strength, bool toKnots, bool flip, VectorStyle vectorStyle) {
  // Barb settings
  ...
  cairo_path_t *cp = cairo_copy_path(cr);
  cairo_new_path(cr);
  cairo_set_line_width(cr, vectorStyle.lineWidth);
  ...
  if (vectorStyle.outlineWidth > 0) {
    CColor outlineColor = vectorStyle.outlineColor;
    ...
    cairo_set_line_width(cr, 1.0 + vectorStyle.outlineWidth);
    cairo_stroke_preserve(cr);
    ...
    cairo_set_line_width(cr, vectorStyle.outlineWidth);
    cairo_stroke_preserve(cr);
  }

  // Stroke thin version
  CColor barbColor = vectorStyle.lineColor;
  cairo_set_antialias(cr, CAIRO_ANTIALIAS_DEFAULT);
  cairo_set_operator(cr, CAIRO_OPERATOR_OVER);
  ...
  if (vectorStyle.drawVectorPlotValue) {
    CColor textColor = vectorStyle.textColor;
    ...
    if (showDirection == false) {
      ...
      CDBDebug("@@@ drawOutline=%d", drawOutline);
      ...
      this->drawStrokedText(x - cos(textDirection + M_PI) * 15 - 5, y + sin(textDirection + M_PI) * 12 + 5, 0, text.c_str(), vectorStyle);
      ...
      this->drawStrokedText(tx, ty, 0, text.c_str(), vectorStyle);
    }
  }
}