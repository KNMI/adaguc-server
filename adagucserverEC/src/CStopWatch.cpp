...
void _StopWatch_Stop(const char *msg){
  ...
  CDBDebug("%.3f ms\t%.3f ms\t%s",stop-start,stop-CSTOPWATCH_H_prevTime ,msg);
  CSTOPWATCH_H_prevTime =stop;
  
}


void StopWatch_Stop(const char *a, ...){
  va_list ap;
  char szTemp[8192+1];
  va_start (ap, a);
