...
std::vector<CT::string> CDBAdapterPostgreSQL::getTableNames(CDataSource *dataSource) {
  ...
  CT::string query;
  query.print("select p.tablename from pg_tables inner join %s as p on pg_tables.tablename = p.tablename where dimension != '' and dimension != 'none' and path=E'P_%s' AND filter=E'F_%s';",
