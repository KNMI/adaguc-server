...
std::vector<CT::string> CDBAdapterPostgreSQL::getTableNames(CDataSource *dataSource) {
  ...
  CT::string query;
  query.print("SELECT p.tablename FROM %s p WHERE path=E'P_%s' AND filter=E'F_%s' and dimension != ''", CDBAdapterPostgreSQL_PATHFILTERTABLELOOKUP, path.c_str(), filter.c_str());
