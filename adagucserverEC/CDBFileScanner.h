...
#define CDBFILESCANNER_LINTDATASET 1024
...
#define CDBFILESCANNER_RETENTIONTYPE_CREATIONDATE "filedate"
...
public:
   ...
   * @param expr: The regular expression to reject/accept each individual file on. When NULL is provided, *.nc is used
   * as default.
   ...
   * @param tailPath: The path under the root path to be scanned.
   * @param _layerPathToScan: When matched with the datasources configured FilePath, the datasource is updated,
   * otherwise it is ignored. This can be used to update a specific layer by defining its corresponding path. When all
   * datasources are looped, only the datasource with matching paths are updated When set to NULL, the datasource is
   * updated always.
