...
const char *CDPPClipMinMax::getId(){
  return "clipminmax";
}
...
template<class T>
void clipData(T* data , size_t size, double min, double max) {
...
int CDPPClipMinMax::execute(CServerConfig::XMLE_DataPostProc* proc, CDataSource* dataSource,int mode){
  ...
  for(size_t varNr=0;varNr<dataSource->getNumDataObjects();varNr++){
    ...
    switch(dataSource->getDataObject(0)->cdfVariable->getType()){
      case CDF_CHAR  :  clipData((char*)d, s, fa, fb);break;
      case CDF_BYTE  :  clipData((char*)d, s, fa, fb);break;
      case CDF_UBYTE :  clipData((unsigned char*)d, s, fa, fb);break;
      case CDF_SHORT :  clipData((short*)d, s, fa, fb);break;
      case CDF_USHORT:  clipData((ushort*)d, s, fa, fb);break;
      case CDF_INT   :  clipData((int*)d, s, fa, fb);break;
      case CDF_UINT  :  clipData((uint*)d, s, fa, fb);break;
      case CDF_FLOAT :  clipData((float*)d, s, fa, fb);break;
      case CDF_DOUBLE:  clipData((double*)d, s, fa, fb);break;
    }
  }
  return 0;
}
