...
CURUniqueRequests::~CURUniqueRequests() {
 ...
 void CURUniqueRequests::nestRequest(it_type_diminfo diminfomapiterator, CURFileInfo *fileInfo, int depth) {
  ...
  void CURUniqueRequests::expandData(CDataSource *dataSource, CDataSource::DataObject *dataObject, CDF::Variable *variable, size_t *start, size_t *count, int d, CURRequest *request, int index) {
    int numberOfDims = dataSource->requiredDims.size();
    if (d < numberOfDims) {
      CT::string dimNameToFind = dataSource->requiredDims[d]->netCDFDimName.c_str();
      // CDBError("Searching %s", dimNameToFind.c_str());
      // CDF::Dimension *dim = variable->getDimensionNE(dimNameToFind.c_str());
      // if (dim == NULL) {
      //   CDBError("Dimension %s not found", dimNameToFind.c_str());
      // }
      ...
      if (count[d] != request->dimensions[requestDimIndex]->values.size()) {
        CDBError("count[d]!=request->dimensions[requestDimIndex]->values.size()");
        throw(__LINE__);
      }

      int multiplier = 1;
      for (size_t j = d + 1; j < size_t(numberOfDims); j++) {
        multiplier *= count[j];
      }
      for (size_t j = 0; j < request->dimensions[requestDimIndex]->values.size(); j++) {
        // CDBDebug(" ***************** %d = %s = %s", d, request->dimensions[requestDimIndex]->name.c_str(),  request->dimensions[requestDimIndex]->values[j].c_str());
        dimensionKeys[d] =  request->dimensions[requestDimIndex]->values[j].c_str();
        expandData(dataSource, dataObject, variable, start, count, d+1, request, j * multiplier + index);
      }
    } else {
  ...
  void CURUniqueRequests::recurDataStructure(CXMLParser::XMLElement *dataStructure, CURResult *result, int depth, int *dimOrdering) {
  ...
  void CURUniqueRequests::createStructure(CDataSource::DataObject *dataObject, CDrawImage *drawImage, CImageWarper *imageWarper, CDataSource *dataSource, int dX, int dY, CXMLParser::XMLElement *gfiStructure) {
...
#endif
              try {
                expandData(dataSource, dataObject, variable, start, count, 0, request, 0);
              } catch (int e) {
