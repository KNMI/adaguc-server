...
private:
  ...
  template <class T> static void drawFunction_nearest(int x, int y, T val, GDWState &warperState, Settings *settings) {
  ...
  template <class T> static void drawFunction_avg_rgb(int x, int y, T val, GDWState &warperState, Settings *settings) {
    if (x >= 0 && y >= 0 && x < (int)settings->width && y < (int)settings->height) {
