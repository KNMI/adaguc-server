...
int main(){
  ...
  input = 0.4; expected = 0.0; result = roundedLinearTransform(input, dfSourceW, dfSourceExtW, dfSourceOrigX, dfDestOrigX, dfDestExtW, dfDestW);
  if (result != expected) {
    CDBError("linearTransform [%f] != [%f] wrong result", result, expected);
    throw __LINE__;
  }
  input = 0.5; expected = 1.0; result = roundedLinearTransform(input, dfSourceW, dfSourceExtW, dfSourceOrigX, dfDestOrigX, dfDestExtW, dfDestW);
  if (result != expected) {
    CDBError("linearTransform [%f] != [%f] wrong result", result, expected);
    throw __LINE__;
  }
  input = -0.5; expected = 0.0; result = roundedLinearTransform(input, dfSourceW, dfSourceExtW, dfSourceOrigX, dfDestOrigX, dfDestExtW, dfDestW);
  if (result != expected) {
    CDBError("linearTransform [%f] != [%f] wrong result", result, expected);
    throw __LINE__;
  }
  input = -0.6; expected = -1.0; result = roundedLinearTransform(input, dfSourceW, dfSourceExtW, dfSourceOrigX, dfDestOrigX, dfDestExtW, dfDestW);
  if (result != expected) {
    CDBError("linearTransform [%f] != [%f] wrong result", result, expected);
    throw __LINE__;
  }
