...
public:
  static int swapPixelsAtLocation(CDataSource *dataSource, CDF::Variable *variable, int mode) {
    ...
    case CDF_CHAR:
      _swapPixelsAtLocation<char>(dataSource, variable, mode);
      break;
    case CDF_BYTE:
      _swapPixelsAtLocation<char>(dataSource, variable, mode);
      break;
    case CDF_UBYTE:
      _swapPixelsAtLocation<uchar>(dataSource, variable, mode);
      break;
    case CDF_SHORT:
      _swapPixelsAtLocation<short>(dataSource, variable, mode);
      break;
    case CDF_USHORT:
      _swapPixelsAtLocation<ushort>(dataSource, variable, mode);
      break;
    case CDF_INT:
      _swapPixelsAtLocation<int>(dataSource, variable, mode);
      break;
    case CDF_UINT:
      _swapPixelsAtLocation<unsigned int>(dataSource, variable, mode);
      break;
    case CDF_FLOAT:
      _swapPixelsAtLocation<float>(dataSource, variable, mode);
      break;
    case CDF_DOUBLE:
      _swapPixelsAtLocation<double>(dataSource, variable, mode);
      break;
    default: {
      CDBError("Unknown data type");
      return 1;
    }
    }
    return 0;
...
private:
  template <class T> static void _swapPixelsAtLocation(CDataSource *dataSource, CDF::Variable *variable, int mode) {

    if (dataReaderDebugEnabled) {
    ...
    while (dataSource->dfBBOX[0] > -180) {
      dataSource->dfBBOX[0] -= dataSource->dfCellSizeX;
    }
    while (dataSource->dfBBOX[0] < -180) {
      dataSource->dfBBOX[0] += dataSource->dfCellSizeX;
    }
...
int CDataReader::parseDimensions(CDataSource *dataSource, int mode, int x, int y, int *gridExtent) {
  ...
  if (!dataSource->formatConverterActive)
    if (CConvertEProfile::convertEProfileData(dataSource, mode) == 0) dataSource->formatConverterActive = true;
  if (!dataSource->formatConverterActive)
    if (CConvertASCAT::convertASCATData(dataSource, mode) == 0) dataSource->formatConverterActive = true;
  if (!dataSource->formatConverterActive)
    if (CConvertUGRIDMesh::convertUGRIDMeshData(dataSource, mode) == 0) dataSource->formatConverterActive = true;
  if (!dataSource->formatConverterActive)
    if (CConvertADAGUCVector::convertADAGUCVectorData(dataSource, mode) == 0) dataSource->formatConverterActive = true;
  if (!dataSource->formatConverterActive)
    if (CConvertADAGUCPoint::convertADAGUCPointData(dataSource, mode) == 0) dataSource->formatConverterActive = true;
  if (!dataSource->formatConverterActive)
    if (CConvertCurvilinear::convertCurvilinearData(dataSource, mode) == 0) dataSource->formatConverterActive = true;
  if (!dataSource->formatConverterActive)
    if (CConvertHexagon::convertHexagonData(dataSource, mode) == 0) dataSource->formatConverterActive = true;
  if (!dataSource->formatConverterActive)
  ...
  if (!dataSource->formatConverterActive)
    if (CConvertH5VolScan::convertH5VolScanData(dataSource, mode) == 0) dataSource->formatConverterActive = true;
  if (!dataSource->formatConverterActive)
  ...
  if (dataSource->formatConverterActive == false) {
    ...
    if (dataReaderDebugEnabled) {
      CDBDebug("[%ld %ld %ld] for %s/%s", sta[0], str[0], sto[0], dataSourceVar->name.c_str(), dataSource->varX->name.c_str());
    }
    ...
    if (dataReaderDebugEnabled) {
      CDBDebug("[%ld %ld %ld] for %s/%s", sta[0], str[0], sto[0], dataSourceVar->name.c_str(), dataSource->varY->name.c_str());
    }
...
int CDataReader::open(CDataSource *dataSource, int mode, int x, int y, int *gridExtent) {
  ...
  if (styleConfiguration != NULL) {
    if (dataSource->stretchMinMaxDone == false) {
      if (styleConfiguration->legendScale == 0.0f && !(styleConfiguration->renderMethod & RM_RGBA))
        dataSource->stretchMinMax = true;
      else
  ...
  } else if (mode == CNETCDFREADER_MODE_OPEN_ALL) {
    if (silent == false) {
  ...
  if (mode == CNETCDFREADER_MODE_OPEN_ALL || mode == CNETCDFREADER_MODE_OPEN_EXTENT) {
    ...
    for (size_t varNr = 0; varNr < dataSource->getNumDataObjects(); varNr++) {
      ...
      if (dataSource->formatConverterActive == false) {
        ...
        if (dataReaderDebugEnabled) {
          CDBDebug("READING DATA FOR varNR [%lu], name=\"%s\"", varNr, dataSource->getDataObject(varNr)->cdfVariable->name.c_str());
          for (size_t d = 0; d < dataSource->getDataObject(varNr)->cdfVariable->dimensionlinks.size(); d++) {
            CDBDebug("%s  \tstart: %lu\tcount %lu\tstride %lu", dataSource->getDataObject(varNr)->cdfVariable->dimensionlinks[d]->name.c_str(), start[d], count[d], stride[d]);
          }
        }
    ...
    if (styleConfiguration->legendOffset != styleConfiguration->legendOffset || styleConfiguration->legendOffset == INFINITY || styleConfiguration->legendOffset == NAN ||
