...
CDF::Variable *CDFObject::getVar(std::string name) {
  if (name == "NC_GLOBAL") {
    return this;
  }
...
CDF::Variable *CDFObject::getVariableThrows(const char *name) {
...
CDF::Variable *CDFObject::getVariableThrows(std::string name) {
  auto var = getVar(name);
  if (var == nullptr) {
    throw(CDF_E_VARNOTFOUND);
  }
...
void ncmlHandleAttribute(xmlNode *cur_node, CT::string NCMLVarName, CDFObject *cdfObject) {
  if (cur_node->properties->name != NULL) {
    ...
    while (node != NULL) {
      ...
      if (strncmp("type", (char *)node->name, 4) == 0) pszAttributeType = (char *)node->children->content;
      if (strncmp("value", (char *)node->name, 5) == 0) pszAttributeValue = (char *)node->children->content;
      if (strncmp("orgName", (char *)node->name, 7) == 0) pszOrgName = (char *)node->children->content;
      node = node->next;
    }
  ...
  if (cur_node->properties->name != NULL) {
    if (cur_node->properties->children->content != NULL) {
      ...
      while (node != NULL) {
        ...
        if (strncmp("orgName", (char *)node->name, 7) == 0) pszOrgName = (char *)node->children->content;
        if (strncmp("type", (char *)node->name, 4) == 0) pszType = (char *)node->children->content;
        if (strncmp("shape", (char *)node->name, 5) == 0) shape = (char *)node->children->content;
        node = node->next;
      }
      // Rename a variable
      ...
      if (pszName != NULL) {
        ...
        if (!shape.empty()) {
          ...
          for (size_t d = 0; d < dims.size(); d++) {
            try {
              CDF::Dimension *dimension = cdfObject->getDimensionThrows(dims[d].c_str());
...
void ncmlHandleDimension(xmlNode *cur_node, CDFObject *cdfObject) {
  if (cur_node->properties->name != NULL) {
    if (cur_node->properties->children->content != NULL) {
      ...
      while (node != NULL) {
        if (strncmp("name", (char *)node->name, 4) == 0) pszName = (char *)node->children->content;
        if (strncmp("orgName", (char *)node->name, 7) == 0) pszOrgName = (char *)node->children->content;
        if (strncmp("length", (char *)node->name, 6) == 0) pszLength = (char *)node->children->content;
        node = node->next;
      }
      // Rename a dimension
      if (pszOrgName != NULL && pszName != NULL) {
      ...
      if (pszName != NULL) {
        ...
        try {
          dim = cdfObject->getDimensionThrows(pszName);
        } catch (...) {
          if (pszOrgName == NULL) {
    ...
    if (pszType != NULL && pszName != NULL) {
      ...
      if (strncmp(pszType, "attribute", 9) == 0) {
        if (attributeParentVarName != NULL) {
          try {
            CDF::Variable *var = cdfObject->getVariableThrows(attributeParentVarName);
...
int CDFObject::getVariableIndexThrows(const char *name) {
...
int CDFObject::getVariableIndexNE(const char *name) {
  if (strncmp("NC_GLOBAL", name, 9) == 0) {
    return -1;
  }
  for (size_t j = 0; j < variables.size(); j++) {
...
CDF::Dimension *CDFObject::addDimension(CDF::Dimension *dim) {
...
int CDFObject::aggregateDim(CDFObject *sourceCDFObject, const char *dimName) {
  CDF::Variable *srcVar;
  ...
  for (size_t v = 0; v < sourceCDFObject->variables.size(); v++) {
    ...
    // if(getVariableNE(srcVar->name.c_str())!=NULL)
    {
      try {
        try {
          destVar = getVariableThrows(srcVar->name.c_str());
        } catch (int e) {
          CDBError("Variable %s not found.", srcVar->name.c_str());
          throw(__LINE__);
        }
