...
CTime::Date CTime::getDate(double offset){
  ...
  date.offset=offset;
  float second;
  if(utCalendar(date.offset,&dataunits,&date.year,&date.month,&date.day,&date.hour,&date.minute,&second)!=0) {
    CDBError("OffsetToAdaguc: Internal error: utCalendar");throw CTIME_CONVERSION_ERROR;
    
  }
  date.second = double(second);
  return date;
}
...
double CTime::dateToOffset( Date date){
  double offset;
  if(utInvCalendar(date.year,date.month,date.day,date.hour,date.minute,date.second,&dataunits,&offset) != 0){
