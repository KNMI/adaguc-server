...
int CDFHDF5Reader::CustomForecastReader::readData(CDF::Variable *thisVar, size_t *start, size_t *count, ptrdiff_t *stride) {
  ...
  status = CDF::copy(thisVar->data, thisVar->getType(), var->data, thisVar->getType(), 0, 0, size);
  if (status != 0) {
...
#ifdef CCDFHDF5IO_DEBUG
              CT::string attrType = CDF::getCDFDataTypeName(var->currentType);
...
#ifdef CCDFHDF5IO_DEBUG
  CT::string typeName = CDF::getCDFDataTypeName(type);
  CDBDebug("Reading %s --> %s with type %s", var->name.c_str(), var->orgName.c_str(), typeName.c_str());
#endif
  char varName[1024];
