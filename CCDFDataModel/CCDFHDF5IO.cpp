...
void CDFHDF5Reader::list(hid_t groupID, const std::string &groupName) {
  ...
  for (int j = 0; j < (int)group_info.nlinks; j++) {
    ...
    if (type == H5I_DATASET) {
      if (CCDFHDF5IO_DEBUG) {
        CDBDebug("H5I_DATASET: %" PRId64 ",%s", groupID, name);
      }
      hid_t datasetID = H5Dopen2(groupID, name, H5P_DEFAULT);
      if (CCDFHDF5IO_DEBUG) {
        CDBDebug("Opened dataset %s with id %" PRId64 " from %" PRId64, name, datasetID, groupID);
      }
      if (datasetID > 0) {
...
void CDFHDF5Reader::closeH5GroupByName(const char *variableGroupName) {
  ...
  while (opengroups.size() > 0) {
    if (CCDFHDF5IO_DEBUG) {
      CDBDebug("closing with id %" PRId64, opengroups.back());
    }
    opengroups.pop_back();
  }
}
