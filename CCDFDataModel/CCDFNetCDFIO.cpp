...
void CDFNetCDFReader::ncError(int, const char *msg, int e) {
...
int CDFNetCDFReader::_readVariableData(CDF::Variable *var, CDFType type, size_t *start, size_t *count, ptrdiff_t *stride) {
  ...
  if (root_id == -1) {
    ...
    if (status != NC_NOERR) {
      ncError(__LINE__, "nc_open: ", status);
      return 1;
    }

    status = nc_inq(root_id, &nDims, &nVars, &nRootAttributes, &unlimDimIdP);
    if (status != NC_NOERR) {
      ncError(__LINE__, "nc_inq: ", status);
      return 1;
    }
  ...
  if (type == CDF_STRING) {
    if (var->isString()) {
      ...
      if (status != NC_NOERR) {
        ncError(__LINE__, "nc_get_var_char (for CDF_STRING): ", status);
      }
      ...
      if (useStartCount == true) {
        if (useStriding) {
          ...
          if (status != NC_NOERR) {
            ncError(__LINE__, "nc_get_vars (typeconversion): ", status);
          }
        } else {
#ifdef CCDFNETCDFIO_DEBUG_OPEN
          ...
          if (status != NC_NOERR) {
            ncError(__LINE__, "nc_get_vara (typeconversion): ", status);
          }
        }
      } else {
#ifdef CCDFNETCDFIO_DEBUG_OPEN
        ...
        if (status != NC_NOERR) {
          ncError(__LINE__, "nc_get_var_string (typeconversion): ", status);
        }
      }
    }
    if (status != NC_NOERR) {
      ...
      ncError(__LINE__, "nc_get_var: ", status);
      return 1;
    }
  ...
  if (type != var->nativeType) {
    ...
    if (useStartCount == true) {
      ...
      if (status != NC_NOERR) {
        ncError(__LINE__, "nc_get_var (typeconversion): ", status);
      }
    }
  ...
  if (type == var->nativeType) {
    if (useStartCount) {
      if (useStriding) {
        ...
        if (status != NC_NOERR) {
          ncError(__LINE__, "nc_get_vars (native): ", status);
        }
      } else {
#ifdef CCDFNETCDFIO_DEBUG_OPEN
        CT::string dims = "";
        ...
        if (status != NC_NOERR) {
          ncError(__LINE__, "nc_get_vara (native): ", status);
        }
      }
    } else {
#ifdef CCDFNETCDFIO_DEBUG_OPEN
      ...
      if (status != NC_NOERR) {
        ncError(__LINE__, "nc_get_var (native): ", status);
      }
    }
    if (status != NC_NOERR) {
      char typeName[254];
...
int CDFNetCDFReader::readDimensions(int groupId, CT::string *groupName) {
  ...
  if (status != NC_NOERR) {
    ...
    ncError(__LINE__, "nc_inq_dimids: ", status);
    return 1;
  }
  int dimIds[nDims];
  ...
  if (status != NC_NOERR) {
    ncError(__LINE__, "nc_inq_ndims: ", status);
    return 1;
  }
  for (int j = 0; j < nDims; j++) {
    ...
    if (status != NC_NOERR) {
      CDBError("For groupName %s: ", groupName->c_str());
      ncError(__LINE__, "nc_inq_dim: ", status);
      return 1;
    }
...
int CDFNetCDFReader::readAttributes(int root_id, std::vector<CDF::Attribute *> &attributes, int varID, int natt) {
  ...
  for (int i = 0; i < natt; i++) {
    ...
    if (status != NC_NOERR) {
      ncError(__LINE__, "nc_inq_attname: ", status);
      return 1;
    }
    ...
    if (attributeExists == false) {
      ...
      if (status != NC_NOERR) {
        ncError(__LINE__, "nc_inq_att: ", status);
        return 1;
      }
      ...
      if (type != NC_STRING) {
        ...
        if (status != NC_NOERR) {
          ncError(__LINE__, "nc_get_att: ", status);
          return 1;
        }
      } else {
...
int CDFNetCDFReader::_findNCGroupIdForCDFVariable(CT::string *varName) {
  ...
  for (size_t j = 0; j < paths.size() - 1; j++) {
    ...
    if (status != NC_NOERR) {
      ncError(__LINE__, "nc_inq_ncid: ", status);
      return -1;
    }
    currentId = grp_ncid;
  }
...
int CDFNetCDFReader::readVariables(int groupId, CT::string *groupName, int mode) {
  ...
  if (status != NC_NOERR) {
    ncError(__LINE__, "nc_inq_grpname_full: ", status);
    return 1;
  }
  ...
  if (nGroups > 0) {
    ...
    if (status != NC_NOERR) {
      ncError(__LINE__, "nc_inq_grps: ", status);
      delete[] groupIds;
      return 1;
    }
    ...
    for (int g = 0; g < nGroups; g++) {
      ...
      if (status != NC_NOERR) {
        ncError(__LINE__, "nc_inq_grpname: ", status);
        delete[] groupIds;
        return 1;
      }
  ...
  if (status != NC_NOERR) {
    ncError(__LINE__, "nc_inq_nvars: ", status);
    return 1;
  }
  for (int j = 0; j < nVars; j++) {
    status = nc_inq_var(groupId, j, flatname, &type, &ndims, dimids, &natt);
    if (status != NC_NOERR) {
      ncError(__LINE__, "nc_inq_var: ", status);
      return 1;
    }
...
void CDFNetCDFWriter::ncError(int line, const char *msg, int e) {
  if (e == NC_NOERR) return;
  char szTemp[1024];
  snprintf(szTemp, 1023, "[E: %s, %d ] %s: %s\n", __FILENAME__, line, msg, nc_strerror(e));
...
int CDFNetCDFWriter::write(const char *fileName, void (*progress)(const char *message, float percentage)) {
  ...
  if (status != NC_NOERR) {
    ...
    ncError(__LINE__, "nc_create: ", status);
    nc_close(root_id);
    root_id = -1;
...
int CDFNetCDFWriter::_write(void (*progress)(const char *message, float percentage)) {
  ...
  for (size_t i = 0; i < cdfObject->attributes.size(); i++) {
    ...
    if (status != NC_NOERR) {
      ...
      ncError(__LINE__, "nc_put_att: ", status);
      return 1;
    }
  }
#ifdef CCDFNETCDFWRITER_DEBUG
  ...
  for (size_t j = 0; j < cdfObject->dimensions.size(); j++) {
    ...
    if (status != NC_NOERR) {
      ncError(__LINE__, "nc_def_dim: ", status);
      delete dim;
      return 1;
    }
      ...
      if ((variable->isDimension == true && writeDimsFirst == 0) || (variable->isDimension == false && writeDimsFirst == 1)) {
        {
          ...
          if (status != NC_NOERR) {
            ...
            ncError(__LINE__, "nc_def_var: ", status);
            return 1;
          }
          if (listNCCommands) {
          ...
          if (netcdfMode >= 4 && numDims > 0 && 1 == 1) {
            ...
            if (variable->dimensionlinks.size() > 2) {
              ...
              //                   for(size_t m=0;m<variable->dimensionlinks.size();m++){
              //                     CDBDebug("ChunkSizes %s =
              //                     %d",variable->dimensionlinks[m]->name.c_str(),chunkSizes[m]);
              //                   }

              status = nc_def_var_chunking(root_id, nc_var_id, 0, chunkSizes);
              if (status != NC_NOERR) {
                ncError(__LINE__, "nc_def_var_chunking: ", status);
                return 1;
              }
            }
          }
          ...
          if (netcdfMode >= 4) {
            ...
            if (variable->dimensionlinks.size() > 0 && variable->currentType != CDF_STRING) {
              ...
              if (status != NC_NOERR) {
                ncError(__LINE__, "nc_def_var_deflate: ", status);
                return 1;
              }
              if (listNCCommands) {
...
// copy data
          ...
          if ((numDims > 0 && writeData == true)) { //||(variable->isDimension&&numDims==1)){
            ...
            if (status != NC_NOERR) {
              ...
              ncError(__LINE__, "nc_enddef: ", status);
              return 1;
            }
            if (listNCCommands) {
...
// CDBDebug("Writing %d elements",variable->getSize());
    ...
    if (status != NC_NOERR) {
      CDBError("For variable %s:", variable->name.c_str());
      ncError(__LINE__, "nc_put_var: ", status);
      return 1;
    }
  }
