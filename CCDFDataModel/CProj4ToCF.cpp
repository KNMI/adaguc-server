...
void CProj4ToCF::initMSGPerspective(CDF::Variable *projectionVariable, std::vector<CProj4ToCF::KVP *> projKVPList) {
  ...
  v = getProj4ValueF("h1", projKVPList, 4.2163970098E7, CProj4ToCF::convertToM);
  projectionVariable->addAttribute(new CDF::Attribute("height_from_earth_center", CDF_FLOAT, &v, 1));
  ...
  v = getProj4ValueF("scale_x", projKVPList, 35785.830098);
  projectionVariable->addAttribute(new CDF::Attribute("scale_x", CDF_FLOAT, &v, 1));
  v = getProj4ValueF("scale_y", projKVPList, -35785.830098);
  projectionVariable->addAttribute(new CDF::Attribute("scale_y", CDF_FLOAT, &v, 1));
...
void CProj4ToCF::initStereoGraphic(CDF::Variable *projectionVariable, std::vector<CProj4ToCF::KVP *> projKVPList) {
  ...
  v = getProj4ValueF("lat_0", projKVPList, 90);
  projectionVariable->addAttribute(new CDF::Attribute("latitude_of_projection_origin", CDF_FLOAT, &v, 1));
  v = getProj4ValueF("lon_0", projKVPList, 0);
  projectionVariable->addAttribute(new CDF::Attribute("straight_vertical_longitude_from_pole", CDF_FLOAT, &v, 1));
  v = getProj4ValueF("lat_ts", projKVPList, 0);
  projectionVariable->addAttribute(new CDF::Attribute("standard_parallel", CDF_FLOAT, &v, 1));
  v = getProj4ValueF("x", projKVPList, 0);
...
void CProj4ToCF::initLCCPerspective(CDF::Variable *projectionVariable, std::vector<CProj4ToCF::KVP *> projKVPList) {
  ...
  v = getProj4ValueF("lat_0", projKVPList, 46.8);
  projectionVariable->addAttribute(new CDF::Attribute("latitude_of_projection_origin", CDF_FLOAT, &v, 1));
  v = getProj4ValueF("lon_0", projKVPList, 2.337229);
  projectionVariable->addAttribute(new CDF::Attribute("longitude_of_central_meridian", CDF_FLOAT, &v, 1));
  v = getProj4ValueF("x_0", projKVPList, 0);
  projectionVariable->addAttribute(new CDF::Attribute("false_easting", CDF_FLOAT, &v, 1));
...
void CProj4ToCF::initLAEAPerspective(CDF::Variable *projectionVariable, std::vector<CProj4ToCF::KVP *> projKVPList) {
  ...
  v = getProj4ValueF("b", projKVPList, 6356755.5, CProj4ToCF::convertToM);
  projectionVariable->addAttribute(new CDF::Attribute("semi_minor_axis", CDF_FLOAT, &v, 1));
  v = getProj4ValueF("pm", projKVPList, 0);
...
void CProj4ToCF::initRPPerspective(CDF::Variable *projectionVariable, std::vector<CProj4ToCF::KVP *> projKVPList) {
  ...
  v = getProj4ValueF("lon_0", projKVPList, 0) - 180.0f;
  projectionVariable->addAttribute(new CDF::Attribute("grid_north_pole_longitude", CDF_FLOAT, &v, 1));
  v = getProj4ValueF("o_lat_p", projKVPList, 47);
  projectionVariable->addAttribute(new CDF::Attribute("grid_north_pole_latitude", CDF_FLOAT, &v, 1));
  v = getProj4ValueF("o_lon_p", projKVPList, 0);
  projectionVariable->addAttribute(new CDF::Attribute("north_pole_grid_longitude", CDF_FLOAT, &v, 1));
...
void CProj4ToCF::initObliqueStereographicPerspective(CDF::Variable *projectionVariable, std::vector<CProj4ToCF::KVP *> projKVPList) {
  ...
  v = getProj4ValueF("lon_0", projKVPList, 0);
  projectionVariable->addAttribute(new CDF::Attribute("longitude_of_central_meridian", CDF_FLOAT, &v, 1));
  v = getProj4ValueF("k", projKVPList, 1);
...
void CProj4ToCF::initGeosPerspective(CDF::Variable *projectionVariable, std::vector<CProj4ToCF::KVP *> projKVPList) {
  ...
  projectionVariable->addAttribute(new CDF::Attribute("semi_major_axis", CDF_FLOAT, &v, 1));
  v = getProj4ValueF("b", projKVPList, 6356752.31414, CProj4ToCF::convertToM);
  projectionVariable->addAttribute(new CDF::Attribute("semi_minor_axis", CDF_FLOAT, &v, 1));
  v = getProj4ValueF("lat_0", projKVPList, 0);
  projectionVariable->addAttribute(new CDF::Attribute("latitude_of_projection_origin", CDF_FLOAT, &v, 1));
  v = getProj4ValueF("lon_0", projKVPList, 0);
...
int CProj4ToCF::convertCFToProj(CDF::Variable *projectionVariable, CT::string *proj4String) {
  ...
  try {
    ...
    if (grid_mapping_name.equals("msgnavigation")) {
      ...
      try {
        projectionVariable->getAttribute("longitude_of_projection_origin")->getDataAsString(&longitude_of_projection_origin);
      } catch (int e) {
      };
      try {
        projectionVariable->getAttribute("latitude_of_projection_origin")->getDataAsString(&latitude_of_projection_origin);
      } catch (int e) {
      };
      try {
        projectionVariable->getAttribute("height_from_earth_center")->getDataAsString(&height_from_earth_center);
      } catch (int e) {
      };
      try {
        projectionVariable->getAttribute("semi_major_axis")->getDataAsString(&semi_major_axis);
      } catch (int e) {
      };
      try {
      ...
      try {
        projectionVariable->getAttribute("perspective_point_height")->getDataAsString(&perspective_point_height);
      } catch (int e) {
      };
      try {
        projectionVariable->getAttribute("semi_major_axis")->getDataAsString(&semi_major_axis);
      ...
      try {
        projectionVariable->getAttribute("sweep_angle_axis")->getDataAsString(&sweep_angle_axis);
      } catch (int e) {
      };
      ...
      try {
        projectionVariable->getAttribute("straight_vertical_longitude_from_pole")->getDataAsString(&straight_vertical_longitude_from_pole);
      } catch (int e) {
      };
      try {
        projectionVariable->getAttribute("latitude_of_projection_origin")->getDataAsString(&latitude_of_projection_origin);
      ...
      try {
        projectionVariable->getAttribute("standard_parallel")->getDataAsString(&standard_parallel);
      } catch (int e) {
      };
      try {
        projectionVariable->getAttribute("semi_major_axis")->getDataAsString(&semi_major_axis);
      ...
      try {
        projectionVariable->getAttribute("false_easting")->getDataAsString(&false_easting);
      } catch (int e) {
      };
      try {
      ...
      proj4String->print("+proj=stere +lat_0=%f +lon_0=%f +lat_ts=%f +a=%f +b=%f +x_0=%f +y_0=%f +units=%s +ellps=WGS84 +datum=WGS84", latitude_of_projection_origin.toDouble(),
      ...
      try {
        projectionVariable->getAttribute("longitude_of_central_meridian")->getDataAsString(&longitude_of_central_meridian);
      } catch (int e) {
      };
      try {
        projectionVariable->getAttribute("latitude_of_projection_origin")->getDataAsString(&latitude_of_projection_origin);
      ...
      proj4String->printconcat(" +lon_0=%f +k_0=1.0 +x_0=%f +y_0=%f +a=%f +b=%f +units=%s", longitude_of_central_meridian.toDouble(), false_easting.toDouble(), false_northing.toDouble(),
                               dfsemi_major_axis, dfsemi_minor_axis, units.c_str());
      ...
      try {
        projectionVariable->getAttribute("longitude_of_prime_meridian")->getDataAsString(&longitude_of_prime_meridian);
      } catch (int e) {
      };
      try {
        projectionVariable->getAttribute("semi_major_axis")->getDataAsString(&semi_major_axis);
      ...
      proj4String->printconcat(" +k_0=1.0 +x_0=%f +y_0=%f +a=%f +b=%f +pm=%f +units=%s", false_easting.toDouble(), false_northing.toDouble(), dfsemi_major_axis, dfsemi_minor_axis,
                               longitude_of_prime_meridian.toDouble(), units.c_str());
      ...
      CT::string false_northing = "0";
      try {
        projectionVariable->getAttribute("grid_north_pole_latitude")->getDataAsString(&grid_north_pole_latitude);
      } catch (int e) {
      };
      try {
        projectionVariable->getAttribute("grid_north_pole_longitude")->getDataAsString(&grid_north_pole_longitude);
      } catch (int e) {
      };
      try {
        projectionVariable->getAttribute("north_pole_grid_longitude")->getDataAsString(&north_pole_grid_longitude);
      } catch (int e) {
      };
      try {
        projectionVariable->getAttribute("semi_major_axis")->getDataAsString(&semi_major_axis);
      ...
      try {
        projectionVariable->getAttribute("central_meridian")->getDataAsString(&longitude_of_central_meridian);
      } catch (int e) {
      };
      // Get scale_factor
      try {
        projectionVariable->getAttribute("scale_factor_at_projection_origin")->getDataAsString(&scale_factor_at_projection_origin);
      } catch (int e) {
      };
      try {
        projectionVariable->getAttribute("scale_factor")->getDataAsString(&scale_factor_at_projection_origin);
      } catch (int e) {
      };
      ...
      proj4String->print("+proj=sterea +lat_0=%f +lon_0=%f +k=%f +a=%f +b=%f +x_0=%f +y_0=%f +units=m +no_defs", latitude_of_projection_origin.toDouble(), longitude_of_central_meridian.toDouble(),
      ...
      proj4String->printconcat(" +lon_0=%f +k_0=1.0 +x_0=%f +y_0=%f +a=%f +units=%s", longitude_of_central_meridian.toDouble(), false_easting.toDouble(), false_northing.toDouble(),
                               semi_major_axis.toDouble(), units.c_str());
      ...
      if (!found) {
        try {
          projectionVariable->getAttribute("longitude_of_projection_origin")->getDataAsString(&longitude_of_central_meridian);
          found = true;
        } catch (int e) {
        };
      }
      ...
      if (!found) {
        try {
          projectionVariable->getAttribute("scale_factor_at_central_meridian")->getDataAsString(&scale_factor_at_projection_origin);
          found = true;
        } catch (int e) {
        };
        if (!found) {
      ...
      proj4String->printconcat(" +lon_0=%f +k_0=%f +x_0=%f +y_0=%f +a=%f +b=%f +units=%s", longitude_of_central_meridian.toDouble(), scale_factor_at_projection_origin.toDouble(),
                               false_easting.toDouble(), false_northing.toDouble(), semi_major_axis.toDouble(), semi_minor_axis.toDouble(), units.c_str());
    } else {
...
int CProj4ToCF::unitTest() {
  ...
  if (__checkProjString("Polar stereographic in M", "+proj=stere +lat_0=90.000000 +lon_0=263.000000 +lat_ts=60.000000 +a=6378140.000000 +b=6356750.000000 +x_0=3475000.000000 +y_0=7475000.000000") !=
      0)
    error = true;
  if (__checkProjString("Lambert conformal conic", "+proj=lcc +lat_0=46.799999 +lat_1=45.898918 +lat_2=47.696011 +lon_0=2.337229 +k_0=1.000000 +x_0=600000.000000 +y_0=2200000.000000") != 0)
    error = true;
  if (__checkProjString("Rotated pole", "+proj=ob_tran +o_proj=0.000000 +lon_0=15.000000 +o_lat_p=47.000000 +o_lon_p=0.000000 +a=6378.140000 +b=6356.750000 +x_0=0.000000 +y_0=0.000000") != 0)
    error = true;
  if (__checkProjString("Oblique stereographic", "+proj=sterea +lat_0=52.156162 +lon_0=5.387639 +k=0.999908 +a=6378140.000000 +b=6356755.500000 +x_0=155000.000000 +y_0=463000.000000 +units=m") != 0)
    error = true;
  ...
  //         if(convertBackAndFort("+proj=stere +lat_0=90.000000 +lon_0=263.000000 +lat_ts=60.000000 +a=6378140.000000 +b=6356750.000000 +x_0=3475000.000000
  //         +y_0=7475000.000000",projectionVariable)!=0){throw(__LINE__);} projectionVariable->getAttribute("proj4_origin")->getDataAsString(&proj4_origin);
  //         projectionVariable->getAttribute("proj4_recons")->getDataAsString(&proj4_recons);
  ...
  //         projectionVariable->getAttribute("proj4_origin")->getDataAsString(&proj4_origin);
  //         projectionVariable->getAttribute("proj4_recons")->getDataAsString(&proj4_recons);
  //         if(!proj4_origin.equals("+proj=ob_tran +o_proj=0.000000 +lon_0=15.000000 +o_lat_p=47.000000 +o_lon_p=0.000000 +a=6378.140000 +b=6356.750000 +x_0=0.000000
  //         +y_0=0.000000")){throw(__LINE__);} if(!proj4_recons.equals("+proj=ob_tran +o_proj=longlat +lon_0=15.000000 +o_lat_p=47.000000 +o_lon_p=0.000000 +a=6378.140000 +b=6356.750000 +x_0=0.000000
  //         +y_0=0.000000 +no_defs")){throw(__LINE__);}
  //
  //         //oblique_stereographic
  //         if(convertBackAndFort("+proj=sterea +lat_0=52.15616055555555 +lon_0=5.38763888888889 +k=0.9999079 +x_0=155000 +y_0=463000 +ellps=bessel +units=m
  //         +no_defs",projectionVariable)!=0){throw(__LINE__);} projectionVariable->getAttribute("proj4_origin")->getDataAsString(&proj4_origin);
  //         projectionVariable->getAttribute("proj4_recons")->getDataAsString(&proj4_recons);
  ...
  //         if(!proj4_recons.equals("+proj=sterea +lat_0=52.156162 +lon_0=5.387639 +k=0.999908 +a=6378140.000000 +b=6356755.500000 +x_0=155000.000000 +y_0=463000.000000 +units=m +no_defs"
  //         )){throw(__LINE__);}
  //
