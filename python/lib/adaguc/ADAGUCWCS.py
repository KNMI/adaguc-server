...
def describeCoverage(adagucexecutable, tmpdir, LOGFILE, WCSURL, COVERAGE, env=None):
    ...
    try:
        ...
        if len(itemlist) != 0:
            ...
            return list(
                daterange(
                    isodate.parse_datetime(start_date),
                    isodate.parse_datetime(end_date),
                    isodate.parse_duration(res_date),
                )
            )
    except:
        pass
...
def iteratewcs(
    ...
    if len(datestodo) == 0:
        raise ValueError(
            "No data found in resource for given dates. Possible date range should be within "
            + str(founddates[0])
            + " and "
            + str(founddates[-1])
        )
    ...
    def makeGetCoverage(single_date, env={}):
        ...
        if CALLBACK == None:
            ...
            CALLBACK(messagetime, ((float(datesdone) / float(numdatestodo)) * 90.0))
    ...
    def monitor2(line):
        ...
        try:
            ...
            if CALLBACK == None:
                print(float(data["percentage"]) * (1.0 / 10) + 90)
            else:
                CALLBACK(data["message"], float(data["percentage"]) * (1.0 / 10) + 90)
        except:
            CALLBACK(line, 50)
