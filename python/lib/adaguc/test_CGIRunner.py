...
def testCGIResponse80px():
   ...
   status, headers = cgiRunner.run(["python3","mockCGIResponse.py", "mockcgidata_headersandcontent80px.bin"],"",output = filetogenerate,env=[])
...
def testCGIResponse8000px():
   ...
   cgiRunner = CGIRunner()
   start = time.perf_counter()
   status, headers = cgiRunner.run(["python3","mockCGIResponse.py", "mockcgidata_headersandcontent8000px.bin"],"",output = filetogenerate,env=[])
   extractedPng = filetogenerate.getvalue()
   end = time.perf_counter()
