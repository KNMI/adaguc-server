...
def get_ref_times_for_coll(metadata) -> list[str]:
    ...
    if (
        ...
        try:
            instance_ids = [
                parse_iso(reft).strftime("%Y%m%d%H%M") for reft in metadata[first_param]["dims"]["reference_time"]["values"].split(",")
            ]
            return instance_ids
        except ValueError:
            pass
...
def get_collectioninfo_from_md(
    ...
    """
    logger.info("get_collectioninfo_from_md(%s, %s) for %s", collection_name, instance, base_url)
    ...
    base_url = base_url + f"/collections/{collection_name}"
...
def get_times_for_collection(metadata, parameter: str = None) -> tuple[list[list[str]], list[str]]:
    """
