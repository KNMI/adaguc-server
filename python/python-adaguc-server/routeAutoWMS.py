...
def handleDataRoute(adagucDataDir, urlParamPath, adagucOnlineResource):
    ...
    if not localPathToBrowse.startswith(adagucDataDir):
        logger.error("Invalid path detected = constructed [%s] from [%s], localPathToBrowse [%s] does not start with adagucDataDir [%s] " % (
    ...
    dataDirectories = [f for f in os.listdir(
        localPathToBrowse) if os.path.isdir(os.path.join(localPathToBrowse, f))]
    dataFiles = [f for f in os.listdir(localPathToBrowse) if os.path.isfile(
        os.path.join(localPathToBrowse, f)) and isFileAllowed(f)]
    data = []
...
def handleAutoWMSDIRRoute(adagucAutoWMSDir, urlParamPath, adagucOnlineResource):
    ...
    localPathToBrowse = os.path.realpath(
        os.path.join(adagucAutoWMSDir, subPath))
    logger.info("localPathToBrowse = [%s]" % localPathToBrowse)
