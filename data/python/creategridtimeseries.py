...
font = ImageFont.truetype("../fonts/OpenSans-Semibold.ttf", 20)

fileOutDir="../datasets/netcdf_5dims_seq1.nc"
dims = {
  'time':{
    'vartype':'d',
    ...
    'values':[netCDF4.date2num(datetime.datetime(2017,01,01,00,00,00), "seconds since 1970-01-01 00:00:00"),
              ...
              netCDF4.date2num(datetime.datetime(2017,01,01,00,15,00), "seconds since 1970-01-01 00:00:00")]
    },
  'height':{
    'vartype':'d',
    'units':"meters",
...
latvar = []
lonvar = []
latvar = list(reversed(range(-90,90)))
lonvar = range(-180,180)
...
ncfile = netCDF4.Dataset(fileOutDir,'w')
...
def Recurse (dims, number, l):
   for value in range(len(dims[dims.keys()[number-1]]['values'])):
    ...
    if number > 1:
      ...
      draw.text((5, 25), fileOutDir, font=font,  fill=255)
